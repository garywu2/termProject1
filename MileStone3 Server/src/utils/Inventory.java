...
/**
 * TODO --- contains method to deal with inventory ---- Perhaps extend
 * dbController
 */
public class Inventory {
	...
	public void decreaseItemQuantity() {
		...
		if (num == 1) {
			...
			System.out.println(
					"Previously, quantity of " + searchedItem.getToolName() + " was " + searchedItem.getToolQuantity());
			searchedItem.setToolQuantity(searchedItem.getToolQuantity() - num);
			System.out.println(
					"Now, quantity of " + searchedItem.getToolName() + " is " + searchedItem.getToolQuantity());
		} else {
	...
	/**
	 * Checks quantity of all items in stock and places orders for those under
	 * quantity of 40
	 * 
	...
	public void checkAllStockQuantity(ArrayList<Order> orderList) {
		...
		for (int i = 0; i < getItemList().size(); i++) {
			if (getItemList().get(i).lowQuantity()) {
				itemsOrdering.add(new Item(getItemList().get(i).getToolId(), getItemList().get(i).getToolName(),
						getItemList().get(i).getToolQuantity(), getItemList().get(i).getToolPrice(),
						getItemList().get(i).getToolSupplier()));
